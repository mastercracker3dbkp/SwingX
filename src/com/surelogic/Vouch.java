 ...
 * <p>
 * When used on a type, method, or constructor declaration, the
 * {@link Vouch} annotation requires a brief programmer explanation for the
 * vouch being made (via the {@code value} attribute).
 * <p>
 ...
 * containable. The object is treated as if it is annotated with
 ...
 * object is treated as if it is annotated with {@link Immutable}. It is a
 * modeling error if the type of the field is primitive. It is also a modeling
 ...
 * object is treated as if it is annotated with {@link ThreadSafe}. It is a
 * modeling error if the type of the field is primitive. It is also a modeling
 * error if the reference type is explicitly annotated {@link NotThreadSafe}.</li>
 * <li><tt>&#064;Vouch(&quot;AnnotationBounds&quot;)</tt> &ndash; (field,
 * local variable, and parameter declarations) Vouches that the
 * constructor, field, local variable, method, or parameter declaration should
 * be treated as though all annotation bounds constraints are met.</li>
 * </ul>
 ...
 * above.
 * 
 * <p>A <tt>&#064;Vouch(&quot;AnnotationBounds&quot;)</tt> annotation is used
 * below to allow <tt>Runnable</tt> to satisfy the conditional bounds
 * of <tt>java.util.ArrayList</tt>, so that
 * <tt>ArrayList&lt;Runnable&gt;</tt> can be considered to be containable.
 * Ultimately this allows the class below to be assured as thread-safe.
 * 
 * <pre>
 * &#064;ThreadSafe
 * &#064;Region("State")
 ...
 * </pre>
 * 
 * <p>The vouch is necessary because <code>java.lang.Runnable</code> is too
 * general a class to be annotated as <code>&#064;ThreadSafe</code> or 
 * <code>&#064;ReferenceObject</code>, as it would need to be to satisfy
 * the conditional bounds required the <code>&#064;Containable</code>
 * annotation of <code>java.util.ArrayList</code>.  In this case, the
 * programmer is vouching that the <code>Runnable</code> objects are not going to
 * do anything that would interfere with the containability of the list
 * object.
 * 
