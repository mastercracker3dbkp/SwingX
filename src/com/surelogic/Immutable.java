 ...
 * instances of the annotated type are always immutable. When used on a field or
 * parameter, this annotation indicates that the field or parameter has an
 * <em>immutable reference</em> (<a href="#immutableRef">see below</a>). When used on a method or
 * constructor, this annotation indicates that the receiver or return value (or
 * both) is an immutable reference.
 * 
 * <h3 id="immutableType">Immutable Types</h3>
 * 
 * <p>
 ...
 * <code>&#064;Mutable</code>.
 * <p>
 * An annotation type declaration may be annotated with <code>&#064;Immutable</code>
 * so that any static fields in the type are verified as immutable. 
 * <p>
 * By default the static part of a class implementation is checked that it is
 * immutable as well as the instance part. It is possible to specify which 
 * portions of the class implementation are checked using the <tt>appliesTo</tt>
 * attribute.  This value can be one of
 * <ul>
 * <li><tt>&#064;Immutable(appliesTo=Part.InstanceAndStatic)</tt> (default) &mdash; The
 * instance part and the static part are checked to be immutable.</li>
 * <li><tt>&#064;Immutable(appliesTo=Part.Instance)</tt> &mdash; The instance
 * part only is checked to be immutable.</li>
 * <li><tt>&#064;Immutable(appliesTo=Part.Static)</tt> &mdash; The static
 * part only is checked to be immutable.</li>
 * </ul>
 * It is a modeling error to explicitly set <tt>appliesTo</tt> in anything but
 * a type declaration.
 * 
 * <h4>Relationship with <code>&#064;ThreadSafe</code></h4>
 * 
 * <p>
 ...
 * <code>&#064;NotThreadSafe</code> is a modeling error because an immutable
 ...
 * </pre>
 * 
 * <p>As an example of treating the static and instance state of a class
 ...
 * </pre>
 * 
 * <h3 id="immutableRef">Immutable References</h3>
 * 
 * <p>
 ...
 * change. In contrast, a {@link ReadOnly} reference may not be used to modify
 ...
 * 
 * <p>
 * <em>Immutable references are not currently checked by analysis.</em>
 * 
...
public @interface Immutable {
   ...
   * class, or both are subject to verification.    This attribute only
   * applies when the annotation is used on a type declaration.
   */
  public Part appliesTo() default Part.InstanceAndStatic;

  /**
   * When annotating a constructor or a method, this attribute must be
   * {@code "this"} or {@code "return"} to clarify the intent that it is the
   * receiver that is immutable. It is a modeling error if the attribute is not
   * {@code "this"} or {@code "return"} in this case. When annotated on a
   * constructor {@code "this"} or {@code "return"} have equivalent semantics.
   * 
   * <pre>
   * class C {
   *   &#064;Immutable(&quot;this&quot;)
